...
impl FileSystem {
  ...
  pub fn add_redirect<'a>(
    ...
    let reroute = FileNode::RerouteUpper(FilePath::new(upper_path.as_ref()));

    self.table_write(|table| {
      let key = lower_path.into();

      table
        .retain_in::<&FilePath<'_>, _>((Bound::Included(&key), Bound::Included(&key)), |_, _| {
          false
        })?;
      Ok(())
    })?;
    ...
    // Self::write().map_with_default(
    //   lower_path.into(),
    //   |node| {
    //     node.reroute.clone_from(&reroute);
    //     node.version += 1;
    //   },
    //   default,
    // );
