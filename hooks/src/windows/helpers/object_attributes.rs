...
use crate::{raw_ptr::UnsafePtrCast, windows::helpers::unicode_string::OwnedUnicodeString};
...
pub trait RawObjectAttrsExt: UnsafePtrCast<OBJECT_ATTRIBUTES> {
  unsafe fn reroute(self, path: impl AsRef<Path>) -> Result<ReroutedObjectAttrs, HookError>;
}

impl<T: UnsafePtrCast<OBJECT_ATTRIBUTES>> RawObjectAttrsExt for T {
  unsafe fn reroute(self, path: impl AsRef<Path>) -> Result<ReroutedObjectAttrs, HookError> {
    unsafe {
